...
def run_project(media_dir: Path, wiki_file: Path, output: Path, mode: str = "auto", probe_media: bool = True, model_name: str = "small", preview: bool = False, corrections_file: Path | None = None, reuse_takes: bool = False) -> dict:
    ...
    if not steps:
        raise ValueError("Wiki 正文中未识别到操作步骤；请保留有序步骤或动作句。")
    ...
    lines = ["# 粗剪审核报告", "", f"- 素材片段：{len(plan['segments'])}", f"- 待确认/未匹配：{sum(s['status'] != 'matched' for s in plan['segments'])}", f"- 未拍摄 Wiki 步骤：{', '.join(plan['missing_step_ids']) or '无'}", "", "## 警告", ""]
